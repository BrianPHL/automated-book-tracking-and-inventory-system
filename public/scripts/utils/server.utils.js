...
export const fetchAccountData = async (type, token) => {
    let result = {};
    try {
...
export const fetchOverviewData = async (type, tab) => {
    let fetchedData = [];
    const Personnel = {
        Dashboard: async () => {
            return new Promise(async (resolve) => {
    ...
    try {
        switch (tab) {
            case 'dashboard':
                ...
                break;
            case 'inventory':
                await Personnel.Inventory();
                break;
            case 'students':
                await Personnel.Students();
                break;
            case 'users':
                await Personnel.Users();
                break;
        }
        return fetchedData;
    }
...
export const fetchTableEntries = async (type, tab, query) => {
    ...
    const Personnel = {
        Dashboard: () => {
            return new Promise(async (resolve) => {
                const fetchedData = await executeDatabaseQuery(!query
                    ?
                        `
                            SELECT 
